...
def SpotCoordinationSite(fasta_dict, mode):
    ...
    if (len(fasta_dict) == len(filtered_dict)):
        mylog(logging.INFO, usrout, "NOTE : All the %d sequences were verified to be valid !"%len(fasta_dict))
    else:
        logging.info("---------------------")
        logging.info(" INVALID SEQUENCE(S)")
        logging.info("---------------------")
    ...
    for header, current_sequence in filtered_dict.items(): # Iterate through the dictionary
        ...
        mylog(logging.INFO, usrout, "-" * 80)
        mylog(logging.INFO, usrout, "WORKING ON THE VALID SEQUENCE "+str(sno)+" . "+header[1:]) 
        mylog(logging.INFO, usrout, "-" * 80)
        mylog(logging.INFO, usrout, current_sequence)
        ...
        if(coordination_site_count > 0):
            ...
            mylog(logging.INFO, usrout, "NOTE : if it's net charge is positive")
            mylog(logging.INFO, usrout, "NOTE : if it's neutral, but it is a CYS BASED motif")
            mylog(logging.INFO, usrout, "NOTE : It's a CP coordinated motif")
            ...
            if (mode=="structure") or (current_sequence_length>MAX_SEQ_LENGTH_FOR_WESA):
                mylog(logging.INFO, usrout, "NOTE : All checks done, preparing tabular summary")
